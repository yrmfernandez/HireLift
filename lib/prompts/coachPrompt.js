...
export function buildCoachUser(jobDescription, extracted, keywords) {
  // The extractor already distilled the job's role, keywords, and required
  // skills into `extracted`, and the gap analysis below captures what's
  // missing. The raw posting is only useful here for extra tone/context, so we
  // trim it hard (from up to ~15k chars) to keep this call well under the TPM
  // limit. The distilled fields carry the real signal.
  const trimmedJd = (jobDescription || "").slice(0, 2500);

  return `JOB DESCRIPTION (trimmed for context):
${trimmedJd}
